...
void *login(struct social_net *start);
int k ;
int n, j ;
int a[20] ;
void main ()
...
struct social_net *create(struct social_net* start)
{
    ...
    for (int i=0 ; i<n ; i++)
    {
        ...
        printf("\n \t Enter Email ID : ");
        scanf(" \t\t%s",(temp->email));
        printf("\n \t Enter your age ( You should be 18 or above to create a SMARTCOMM Account ): ") ;
        scanf ("\t\t%d",&(temp->age)) ;
      while((temp->age)<18)
        {
            printf("\n\t\tYou must be 18 years old to create smartcom account\n");
...
void *login(struct social_net *start)
{
 ...
 scanf("%s",&passwordl) ;
 if(strcmp(usernamel,start->username)==0 && strcmp(passwordl,start->password)==0)
 {
    printf("Welcome to the SMARTCOMM") ;
    do
    {
    printf("\n Would you like to try our new feature !!!!!") ;
    printf("\n What would you like to do !!!") ;
    printf("\n Not now . Some other time Press 0 to return to main maenu") ;
    printf("\n 1. Create Family age tree ") ;
    scanf("%d",&ch) ;
    switch (ch)
    {
        case 1 :
 ...
 while(p->connect!=NULL)
 {
     if(strcmp(usernamel,p->connect->username)==0 && strcmp(passwordl,p->connect->password)==0)
     {
             printf("Welcome to the SMARTCOMM") ;
             do
             {
...
struct social_net *insert(struct social_net *start)
{
    ...
    if(pos==1)
    {
       ...
       while((temp->age)<18)
        {
            printf("You must be 18 years old to create smartcom account\n");
            printf("\n \t Please reenter your age : ") ;
            scanf ("\t\t%d",&(temp->age)) ;
        }
        printf("\t\t\nEnter email:");
        scanf("%s",(temp->email));
